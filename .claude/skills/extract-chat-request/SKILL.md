...
2. Ensure `.claude/network-captures/` exists (gitignored).
3. Spawn one `general-purpose` subagent, `model: "haiku"`, `run_in_background: false`. Prompt must be fully self-contained and include:
   ...
   - Call `get_network_request` with that `reqid`, `requestFilePath: .claude/network-captures/<reqid>-request`, `responseFilePath: .claude/network-captures/<reqid>-response`. (Tool appends `.network-request`/`.network-response` itself — use those exact final names.)
