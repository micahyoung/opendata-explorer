...
* **Schema Scope:** To prevent context-window exhaustion and hallucination, the app will launch with a hardcoded, highly curated declarative dictionary of high-value datasets, each defined in its own config file with schema and worked question→SoQL examples. Only the dataset's identity (ID, name, description) is always in context; its field schema and exemplars are retrieved by the LLM on demand, one dataset (or a handful, for comparisons) at a time, so growing the curated catalog doesn't grow the cost of every turn. **v1 ships with fourteen datasets: 311 Service Requests (`erm2-nwe9`), the 2015 Street Tree Census (`uvpi-gqnh`), MTA Bus Automated Camera Enforcement (ACE) Violations (`kh8p-hcbm`), the SF Street Tree List (`tkzw-k3nq`), the Austin Tree Inventory (`wrik-xasw`), Cincinnati 311 (Non-Emergency) Service Requests (`gcej-gmiw`), Chicago 311 Service Requests (`v6vf-nfxy`), SF 311 Cases (`vw6y-z8j6`), MyLA311 Service Request Data 2025 (`h73f-gn57`), Seattle Customer Service Requests (`5ngg-rpne`), Austin 311 Public Data (`xwdj-i9he`), Calgary 311 Service Requests (`iahh-g8bj`), Honolulu 311 Reports (`6hui-dvrh`), and Baton Rouge 311 Citizen Requests (`7ixm-mnvx`)** — the ACE dataset covers bus lane, bus stop, and double-parking violations captured under NY State's expanded bus camera enforcement law, the SF and Austin tree datasets cover street trees maintained by their respective cities, the Cincinnati dataset is the catalog's first `latlon`-mode dataset (separate `latitude`/`longitude` number columns, no native point type), MyLA311's dataset ID is year-partitioned (LA publishes a new resource ID annually, so this ID will need to be swapped for the next year's once 2025 stops updating), Calgary is the catalog's first dataset outside the US, which is why the Nominatim geocoding step no longer hard-restricts results to US addresses, and Honolulu is a small, recently-launched dataset (well under 1,000 rows). These are published across eleven different Socrata domains — `data.cityofnewyork.us`, `data.ny.gov`, `data.sfgov.org`, `data.austintexas.gov`, `data.cincinnati-oh.gov`, `data.cityofchicago.org`, `data.lacity.org`, `data.seattle.gov`, `data.calgary.ca`, `data.honolulu.gov`, and `data.brla.gov` — so curated datasets are no longer assumed to live on a single city- or state-run domain. Additional datasets (e.g., restaurant inspections, NYPD complaints, parks) are deferred to later iterations. Dynamic catalog search is deferred indefinitely. A dataset only qualifies for curation if it's a genuine SoQL-queryable Socrata table — some catalog listings are merely federated pointers to an external GIS backend (no real rows behind the Socrata API), and those are excluded regardless of how promising the listing looks.
